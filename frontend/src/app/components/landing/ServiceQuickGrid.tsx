...
export default function ServiceQuickGrid({ onGoCopilot, onComingSoon }: ServiceQuickGridProps) {
  ...
  return (
    <section className="px-6 md:px-12 -mt-8 relative z-20 shrink-0">
      <div className="max-w-6xl mx-auto bg-white border border-border-slate rounded-2xl shadow-xl p-6 md:p-8 grid grid-cols-2 md:grid-cols-6 gap-6 text-center">
        {items.map(({ icon: Icon, label, onClick }) => (
          <button
            key={label}
            onClick={onClick}
            className="flex flex-col items-center justify-center p-3 rounded-xl hover:bg-gov-cream transition-all group"
          >
