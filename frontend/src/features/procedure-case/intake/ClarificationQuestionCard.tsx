  ...
  return (
    <div className="mx-4 mb-3 space-y-3">
        ...
        <ul className="space-y-1.5">
          {priorAnswers.map((a) => {
            ...
            return (
              <li
                key={a.questionId}
                className="flex items-center justify-between gap-2 px-3 py-2 bg-neutral-bg border border-border-slate rounded-lg text-xs"
              >
                <span className="text-foreground/70 font-medium truncate">
                  {q?.prompt}: <span className="font-bold text-primary">{a.value}</span>
                </span>
                <button
                  type="button"
                  onClick={() => onEdit(a.questionId)}
                  className="shrink-0 text-accent font-bold hover:text-accent-hover focus-visible:ring-2 focus-visible:ring-accent rounded outline-none"
                >
      ...
      <div
        ...
        className="p-4 bg-card-bg border border-border-slate rounded-xl shadow-sm space-y-3 text-left"
      >
        <div>
          <span className="text-[10px] font-bold text-accent tracking-wider uppercase">
            Câu hỏi {currentIndex + 1}/{questions.length}
          </span>
          <h4 id={headingId} className="text-sm font-bold text-primary mt-0.5">
            {question.prompt}
            {question.required && <span className="text-error"> *</span>}
          </h4>
          {question.why && <p className="text-xs text-zinc-500 mt-1">{question.why}</p>}
        </div>
        ...
        {question.options.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {question.options.map((opt, i) => (
              <button
                ...
                className="px-3.5 py-2 border border-border-slate rounded-lg text-xs font-semibold text-primary hover:border-accent hover:bg-neutral-bg transition-all focus-visible:ring-2 focus-visible:ring-accent outline-none"
              >
                {opt}
              </button>
            ))}
          ...
          >
            <input
              ...
              className="flex-1 px-3 py-2 border border-border-slate bg-card-bg rounded-md text-xs focus:outline-none focus:border-accent focus-visible:ring-2 focus-visible:ring-accent"
            />
            <button
              type="submit"
              className="px-4 py-2 bg-accent text-white text-xs font-bold rounded-md hover:bg-accent-hover transition-all"
            >
              Tiếp tục
            </button>
          </form>
