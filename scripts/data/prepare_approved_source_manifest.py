"""Create an approved-source manifest template from K1-approved fixture rows."""
...
def _require_artifacts_path(path: Path, repository_root: Path, parser: argparse.ArgumentParser) -> Path:
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Approved source manifest template written: rows={len(rows)} path={output_path}")
    return 0
