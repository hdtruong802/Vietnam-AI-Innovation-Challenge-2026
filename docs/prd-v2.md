...
| A4 | Existing system | Chưa có hệ thống nào đang chạy; tích hợp Cổng DVCQG thật ngoài scope MVP (chỉ widget/headless API làm đường tích hợp) | Proposal §5, D-008 web-first |
| A5 | Revenue | MVP không có mục tiêu doanh thu trực tiếp; business goal đo bằng pilot pathway và giá trị vận hành (giảm hồ sơ thiếu, giảm câu hỏi lặp) | Rubric Business Viability trong proposal |
| A6 | Dữ liệu demo | 100% synthetic; không hồ sơ công dân thật trong demo/eval | Chính sách PII trong proposal §6 |
| A7 | Ngôn ngữ | MVP chỉ tiếng Việt; đa ngôn ngữ là roadmap | Out of Scope |
...
*Nguồn sự thật về scope/trust model là [`proposal.md`](./proposal.md). Mọi thay đổi shared API / pack schema / demo flow cần Task Record + Decision Log theo team protocol. Các giá trị `TBD`/Assumption phải được chốt bằng Decision trước khi implement phần tương ứng.*
